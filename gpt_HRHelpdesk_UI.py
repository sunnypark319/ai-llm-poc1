...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🔍 검색 테스트"):
            if st.session_state.documents_loaded:
                test_query = st.text_input("테스트 검색어:", value="연차 신청")
                if test_query:
    ...
    # 대화 히스토리 표시 (chat_message 사용으로 변경)
    ...
    # 사용자 입력 처리 (chat_input 사용)
    if prompt := st.chat_input("질문을 입력하세요... (예: 연차 신청 방법이 궁금해요)"):
        # 사용자 메시지 즉시 표시
        with st.chat_message("user"):
            st.write(prompt)
    ...
    if not st.session_state.conversation_history:
        with st.chat_message("assistant", avatar="👥"):
            st.write("안녕하세요! HR 헬프데스크입니다. 인사 관련 문의사항이 있으시면 언제든 말씀해주세요! 😊")
            st.info("💡 **팁:** 왼쪽 사이드바에서 HR 관련 문서를 업로드하시면 더 정확한 답변을 받을 수 있습니다.")
